...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = this.tail.filterAcc(p, if (p(elem)) acc.incl(elem) else acc)
  ...
  def compareRetweetCount(tw1: Tweet, tw2: Tweet) = (tw1.retweets > tw2.retweets)

  def descendingByRetweet: TweetList = descendingByRetweetAcc(this, Nil)

  def descendingByRetweetAcc(source: TweetSet, acc: TweetList): TweetList = {
...
object Nil extends TweetList {
  ...
  def isEmpty = true
  def +(tw: Tweet) = new Cons(tw, Nil)
}

class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  def +(tw: Tweet) = new Cons(head, tail + tw)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(p => google.exists(token => p.text.contains(token)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(p => apple.exists(token => p.text.contains(token)))
